...
struct CounterAPI {
    ...
    func updateCounterPublisher(id: String, value: Int) -> AnyPublisher<[Counter], Error> {
            let endpoint = (value > 0) ? "inc" : "dec"
        
            let session = URLSession(configuration: URLSessionConfiguration.default, delegate: nil, delegateQueue: nil)
            let url = baseURL.appendingPathComponent("counter/\(endpoint)")
